...
// Find and replace - add button before the "Đặt lịch KD" button
content = content.replace(
    /(\s+<Button\s+size="sm"\s+className="bg-emerald-600 hover:bg-emerald-700 text-white"\s+onClick=\{\(\) => setInspectionSystemOpen\(true\)\})/,
    buttonToAdd + '$1'
);
...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('Inspection System Dialog')) {
        insertIndex = i;
        break;
    }
}
...
if (insertIndex > 0) {
    lines.splice(insertIndex, 0, ...dialogToAdd.split('\n'));
    content = lines.join('\n');
}
...
console.log('Successfully added detail button and dialog to e2e-management.tsx');
