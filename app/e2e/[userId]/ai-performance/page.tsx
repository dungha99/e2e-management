...
function AiPerformanceContent({ userId }: { userId: string }) {
  ...
  const handleAccountChange = (newUserId: string) => {
    ...
    router.push(`/e2e/${newUserId}/ai-performance?${params.toString()}`)
  }
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 flex-1 w-full">
        ...
        {error && !data ? (
          ...
          <AiFunnelDashboard 
            data={data} 
            loading={loading}
            filters={{ dateRange, picId: userId }}
...
export default function AiPerformancePage({ params }: { params: Promise<{ userId: string }> }) {
  const { userId } = use(params)

  return (
    <Suspense fallback={<div className="flex items-center justify-center min-h-screen">Loading...</div>}>
      <AiPerformanceContent userId={userId} />
    </Suspense>
  )
}
