...
import { Loader2, User, PhoneCall, Pencil, Clock, RefreshCw, Star, Zap, MessageSquare, Car, Images, MapPin, Send, Activity, ArrowLeft, Bell, Upload, Settings2, Bot } from "lucide-react"
...
interface LeadDetailPanelProps {
  ...
  // Tab State
  activeDetailView: "workflow" | "agent-tracing" | "agent-prompts" | "decoy-web" | "recent-activity" | "decoy-history"
  onActiveDetailViewChange: (view: "workflow" | "agent-tracing" | "agent-prompts" | "decoy-web" | "recent-activity" | "decoy-history") => void
      ...
      <div className={`flex-1 overflow-y-auto scroll-touch scrollbar-hide ${isMobile ? 'has-bottom-bar' : ''}`}>
        ...
        <div className={`px-2 sm:px-4 md:px-6 lg:px-8 pt-2 sm:pt-4 md:pt-6 pb-2 sm:pb-4 md:pb-6 border-b bg-gray-50  ${isMobile ? 'bg-white' : 'sticky top-0 z-10'}`}>
          ...
          <div className="flex items-center gap-2 sm:gap-4 md:gap-6 border-b -mb-3 sm:-mb-4 md:-mb-6 overflow-x-auto scrollbar-none pb-px bg-gray-50">
            ...
            <button
              type="button"
              className={`px-2 sm:px-4 py-2 text-xs sm:text-sm font-medium border-b-2 transition-colors whitespace-nowrap flex-shrink-0 ${activeDetailView === "agent-prompts"
                ? "text-violet-600 border-violet-600"
                : "text-gray-500 border-transparent hover:text-gray-700"
                }`}
              onClick={() => onActiveDetailViewChange("agent-prompts")}
            >
              <div className="flex items-center gap-1.5 sm:gap-2">
                <Bot className="h-3.5 sm:h-4 w-3.5 sm:w-4" />
                <span className="hidden sm:inline">Agent Prompts</span>
        ...
        {activeDetailView === "agent-prompts" && (
          <div className="h-[calc(100vh-250px)] overflow-y-auto scrollbar-hide">
            <AgentPromptsTab picId={selectedLead.pic_id} />
          </div>
        )}
