  ...
  const walkObject = (obj: any) => {
    ...
    if (isBidding) {
      ...
      steps.push({
        ...
        connectorId: CONNECTOR_MAP.script.connectorId,
        connectorLabel: "Gửi Script",
        rawContext: `YÊU CẦU BẮT BUỘC: Generate parameters với mảng messages chính xác chứa 1 phần tử là: "dạ e gửi link phiên đấu giá ạ: https://vucar.vn/phien-dau-gia/tin-xe/{{cars.slug}}" (giữ nguyên từng chữ, không di dịch, chỉ thay đổi slug thành giá trị sku chính xác của xe).`,
